...
digitalZoomspeed.oninput = () => {
    PTZ.DIGITALzoomSpeed = digitalZoomspeed.value;
    digitalZoomspeed.textContent = digitalZoomspeed.value;
};
...
//  video.style.transformOrigin =`${posX*100}% ${posY*100}%`;
...
// zoomIn.onclick=()=>fetch("/api/zoom",{method:"POST",body:"in"});
// zoomOut.onclick=()=>fetch("/api/zoom",{method:"POST",body:"out"});
// zoomStop.onclick=()=>fetch("/api/zoom",{method:"POST",body:"stop"});
...
// btn.onmousedown=()=>{
// console.log("[event]  joystick" ,btn); 
// fetch("/api/ptz",{
// method:"POST",
...
function zoomStart(action){
    fetch(`/api/ptz/zoom?action=${action}&zs=${PTZ.zoomSpeed}`);
}
...
/* Mouse */
zoomInBtn.onmousedown  = ()=>zoomStart("ZOOMIN");
zoomOutBtn.onmousedown = ()=>zoomStart("ZOOMOUT");

zoomInBtn.onmouseup =
zoomOutBtn.onmouseup = zoomStop;

/* Touch */
zoomInBtn.ontouchstart  = ()=>zoomStart("ZOOMIN");
zoomOutBtn.ontouchstart = ()=>zoomStart("ZOOMOUT");

zoomInBtn.ontouchend =
zoomOutBtn.ontouchend = zoomStop;


const digitalSlider = document.getElementById("digitalZoom");

if (digitalSlider) {
    digitalSlider.oninput = () => {

        PTZ.digitalZoom = parseFloat(digitalSlider.value);

        video.style.transform =
            `scale(${PTZ.digitalZoom})`;
    };
}
...
video.addEventListener("wheel", e => {
    ...
    PTZ.digitalZoom += e.deltaY < 0 ? 0.1 : -0.1;
    PTZ.digitalZoom = Math.max(1, Math.min(5, PTZ.digitalZoom));

    digitalSlider.value = PTZ.digitalZoom;
...
/* sliders */
brightness.oninput = ()=>setImage("BRIGHT",brightness.value);
contrast.oninput   = ()=>setImage("CONTRAST",contrast.value);
saturation.oninput = ()=>setImage("SATURATION",saturation.value);
...
//      liveFeed.pause();  // 2️⃣ Pause video
//   } else {
//     console.log("👀 TAB VISIBLE → resume live");
//    try {
