...
//         }
//       });
//       this.toggleNetworkSections(sbc.CONNECT_SYSTEM_NETWORK);

//       // Determine WiFi and Ethernet modes from the config
//       // If SYSTEM_NETWORK_MODE is STATIC, check which IPs are populated
//       let wifiMode = 'DHCP';
//       let ethernetMode = 'DHCP';
      
//       if (sbc.SYSTEM_NETWORK_MODE === 'STATIC') {
//         // If we have separate WiFi/Ethernet static IPs, set modes individually
//         if (sbc.STATIC_IP_ADDRESS_WIFI) {
//           wifiMode = 'STATIC';
//         }
//         if (sbc.STATIC_IP_ADDRESS_ETHERNET) {
//           ethernetMode = 'STATIC';
//         }
//       }

//       // Set WiFi Network Mode
//       const wifiModeRadios = document.querySelectorAll('input[name="wifiNetworkMode"]');
//       wifiModeRadios.forEach(radio => {
//         if (radio.value === wifiMode) {
//           radio.checked = true;
//         }
//       });
//       this.toggleWifiStaticIP(wifiMode);

//       // Set Ethernet Network Mode
//       const ethernetModeRadios = document.querySelectorAll('input[name="ethernetNetworkMode"]');
//       ethernetModeRadios.forEach(radio => {
//         if (radio.value === ethernetMode) {
//           radio.checked = true;
//         }
//       });
...
//       this.toggleEthernetStaticIP(ethernetMode);

//       // All Network Fields
//       this.setFieldValue('sbc_systemMacAddress', sbc.SYSTEM_MAC_ADDRESS);
//       this.setFieldValue('sbc_subnetMask', sbc.SUBNET_MASK);
//       this.setFieldValue('sbc_gateway', sbc.GATE_WAY);
//       this.setFieldValue('sbc_dns', sbc.DNS_Address);
      
//       // WiFi-specific fields
//       this.setFieldValue('sbc_staticIP_wifi', sbc.STATIC_IP_ADDRESS_WIFI || "");
//       this.setFieldValue('sbc_dynamicIP_wifi', sbc.DYNAMIC_IP_ADDRESS_WIFI || "");
//       this.setFieldValue('sbc_wifiSSID', sbc.WIFI_SSID);
//       this.setFieldValue('sbc_wifiPassword', sbc.WIFI_PASSWORD);
//       this.setFieldValue('sbc_macAddress_wifi', sbc.WIFI_MAC_Address);
      
//       // Ethernet-specific fields
//       this.setFieldValue('sbc_staticIP_ethernet', sbc.STATIC_IP_ADDRESS_ETHERNET || "" );
//       this.setFieldValue('sbc_dynamicIP_ethernet', sbc.DYNAMIC_IP_ADDRESS_ETHERNET || "");
//       this.setFieldValue('sbc_macAddress_ethernet', sbc.ETHERNET_MAC_Address);
...
//   // }
// ////////////////////////////////////
//   // Toggle WiFi/Ethernet Sections
//   toggleNetworkSections(type) {
//     const wifiSection = document.getElementById('wifiSettingsSection');
//     const ethernetSection = document.getElementById('ethernetSettingsSection');
    
//     if (type === 'WIFI') {
//       if (wifiSection) wifiSection.style.display = 'block';
//       if (ethernetSection) ethernetSection.style.display = 'none';
//     } else if (type === 'ETHERNET') {
//       if (wifiSection) wifiSection.style.display = 'none';
//       if (ethernetSection) ethernetSection.style.display = 'block';
//     } else if (type === 'ETHERNET & WIFI') {
//       if (wifiSection) wifiSection.style.display = 'block';
//       if (ethernetSection) ethernetSection.style.display = 'block';
//     }
//   }

//   // Toggle WiFi Static IP Settings
//   toggleWifiStaticIP(mode) {
//     const staticField = document.getElementById('sbc_staticIP_wifi');
//     if (staticField) {
//       staticField.disabled = mode !== 'STATIC';
//       staticField.style.opacity = mode === 'STATIC' ? '1' : '0.6';
//     }
//   }

//   // Toggle Ethernet Static IP Settings
//   toggleEthernetStaticIP(mode) {
//     const staticField = document.getElementById('sbc_staticIP_ethernet');
//     if (staticField) {
//       staticField.disabled = mode !== 'STATIC';
//       staticField.style.opacity = mode === 'STATIC' ? '1' : '0.6';
//     }
...
//     // SBC System Network - EXACT field names with separate WiFi/Ethernet IPs

//         const connectionType = document.querySelector('input[name="sbcNetworkType"]:checked')?.value || 'WIFI';
//         const wifiMode = document.querySelector('input[name="wifiNetworkMode"]:checked')?.value || 'DHCP';
//         const ethernetMode = document.querySelector('input[name="ethernetNetworkMode"]:checked')?.value || 'DHCP';
        
//         // Determine overall system mode

//         let systemNetworkMode = 'DHCP';

//         if (connectionType === 'WIFI' && wifiMode === 'STATIC') {
//           systemNetworkMode = 'STATIC';
//         }else if (connectionType === 'ETHERNET' && ethernetMode === 'STATIC') {
//           systemNetworkMode = 'STATIC';
//         } else if (connectionType === 'ETHERNET & WIFI' && (wifiMode === 'STATIC' || ethernetMode === 'STATIC')) {
//           systemNetworkMode = 'STATIC';
//         }
...
//           SYSTEM_MAC_ADDRESS: document.getElementById('sbc_systemMacAddress')?.value || '',
//           CONNECT_SYSTEM_NETWORK: connectionType,
...
//             // Separate Ethernet IPs
//           SYSTEM_NETWORK_ETHERNET_MODE: wifiMode,
//           STATIC_IP_ADDRESS_ETHERNET: document.getElementById('sbc_staticIP_ethernet')?.value || '',
//           DYNAMIC_IP_ADDRESS_ETHERNET: document.getElementById('sbc_dynamicIP_ethernet')?.value || '',
//           ETHERNET_MAC_Address: document.getElementById('sbc_macAddress_ethernet')?.value || '',


//           // Separate WiFi IPs
//           SYSTEM_NETWORK_WIFI_MODE: wifiMode,
//           STATIC_IP_ADDRESS_WIFI: document.getElementById('sbc_staticIP_wifi')?.value || '',
//           DYNAMIC_IP_ADDRESS_WIFI: document.getElementById('sbc_dynamicIP_wifi')?.value || '',
//           WIFI_MAC_Address : document.getElementById('sbc_macAddress_wifi')?.value || '',
//           WIFI_AVAILABLE_LIST: [],
//           WIFI_SSID: document.getElementById('sbc_wifiSSID')?.value || 'HEMANT',
//           WIFI_PASSWORD: document.getElementById('sbc_wifiPassword')?.value || ''
...
//   async applyNetworkChanges(networkConfig) {
//     try {
//       // Prepare the full network configuration with interface-specific settings
//       const applyConfig = {
//         connectionType: networkConfig.CONNECT_SYSTEM_NETWORK,
//         wifiConfig: null,
//         ethernetConfig: null
//       };
...
//         const wifiMode = document.querySelector('input[name="wifiNetworkMode"]:checked')?.value || 'DHCP';
//         applyConfig.wifiConfig = {
//           mode: wifiMode,
//           staticIP: networkConfig.STATIC_IP_ADDRESS_WIFI,
//           ssid: networkConfig.WIFI_SSID,
//           password: networkConfig.WIFI_PASSWORD,
//           gateway: networkConfig.GATE_WAY,
//           subnet: networkConfig.SUBNET_MASK,
//           dns: networkConfig.DNS_Address
//         };
//       }
      
//       // Ethernet Configuration
//       if (networkConfig.CONNECT_SYSTEM_NETWORK === 'ETHERNET' || 
//           networkConfig.CONNECT_SYSTEM_NETWORK === 'ETHERNET & WIFI') {
//         const ethernetMode = document.querySelector('input[name="ethernetNetworkMode"]:checked')?.value || 'DHCP';
//         applyConfig.ethernetConfig = {
//           mode: ethernetMode,
//           staticIP: networkConfig.STATIC_IP_ADDRESS_ETHERNET,
//           gateway: networkConfig.GATE_WAY,
//           subnet: networkConfig.SUBNET_MASK,
//           dns: networkConfig.DNS_Address
//         };
...
//         method: 'POST',
//         headers: { 'Content-Type': 'application/json' },
//         body: JSON.stringify(applyConfig)
//       });
...
class ConfigController {
  ...
  populateFormFields() {
    ...
    if (this.config.SBC_SYSTEM_NETWORK) {
      ...
      if (sbc.SYSTEM_NETWORK_ETHERNET_MODE === 'STATIC'  || sbc.SYSTEM_NETWORK_WIFI_MODE === 'STATIC'  ) {
        // If we have separate WiFi/Ethernet static IPs, set modes individually
        if (sbc.SYSTEM_NETWORK_WIFI_MODE) {
            wifiMode = 'STATIC';
        }
        if (sbc.SYSTEM_NETWORK_ETHERNET_MODE) {
          ethernetMode = 'STATIC';
        }
      }
      ...
      // WiFi-specific fields
      this.setFieldValue('sbc_staticIP_wifi', sbc.STATIC_IP_ADDRESS_WIFI || sbc.STATIC_IP_ADDRESS);
      this.setFieldValue('sbc_dynamicIP_wifi', sbc.DYNAMIC_IP_ADDRESS_WIFI || sbc.DYNAMIC_IP_ADDRESS);
      this.setFieldValue('sbc_wifiSSID', sbc.WIFI_SSID);
      this.setFieldValue('sbc_wifiPassword', sbc.WIFI_PASSWORD);
      this.setFieldValue('sbc_macAddress_wifi', sbc.WIFI_MAC_Address);
      
      // Ethernet-specific fields
      this.setFieldValue('sbc_staticIP_ethernet', sbc.STATIC_IP_ADDRESS_ETHERNET || sbc.STATIC_IP_ADDRESS);
      this.setFieldValue('sbc_dynamicIP_ethernet', sbc.DYNAMIC_IP_ADDRESS_ETHERNET || sbc.DYNAMIC_IP_ADDRESS);
      this.setFieldValue('sbc_macAddress_ethernet', sbc.ETHERNET_MAC_Address);
  ...
  setupEventListeners() {
    ...
    document.querySelectorAll('input[name="sbcNetworkType"]').forEach(radio => {
      radio.addEventListener('change', (e) => {
        this.toggleNetworkSections(e.target.value);
      });
    });

    // WiFi Network Mode Switch
    document.querySelectorAll('input[name="wifiNetworkMode"]').forEach(radio => {
      radio.addEventListener('change', (e) => {
        this.toggleWifiStaticIP(e.target.value);
      });
    });

    // Ethernet Network Mode Switch
    document.querySelectorAll('input[name="ethernetNetworkMode"]').forEach(radio => {
      radio.addEventListener('change', (e) => {
        this.toggleEthernetStaticIP(e.target.value);
      });
    });
...
SBC_SYSTEM_NETWORK : {
          ...
          DNS_Address: document.getElementById('sbc_dns')?.value || '8.8.8.8',
            ...
            // Separate Ethernet IPs
          SYSTEM_NETWORK_ETHERNET_MODE: ethernetMode,
          STATIC_IP_ADDRESS_ETHERNET: document.getElementById('sbc_staticIP_ethernet')?.value || '',
          DYNAMIC_IP_ADDRESS_ETHERNET: document.getElementById('sbc_dynamicIP_ethernet')?.value || '',
          ETHERNET_MAC_Address: document.getElementById('sbc_macAddress_ethernet')?.value || '',
      ...
      //     SYSTEM_MAC_ADDRESS: document.getElementById('sbc_systemMacAddress')?.value || '',
      //     CONNECT_SYSTEM_NETWORK: connectionType,
      //     SYSTEM_NETWORK_MODE: systemNetworkMode,
          
      //     // General static IP (for backward compatibility)
      //     STATIC_IP_ADDRESS: document.getElementById('sbc_staticIP_wifi')?.value || 
      //                        document.getElementById('sbc_staticIP_ethernet')?.value || 
      //                        '192.168.29.15',
      ...
      //     // General dynamic IP (shows current active one)
      //     DYNAMIC_IP_ADDRESS: document.getElementById('sbc_dynamicIP_wifi')?.value || 
      //                         document.getElementById('sbc_dynamicIP_ethernet')?.value || '',
          
      //     GATE_WAY: document.getElementById('sbc_gateway')?.value || '192.168.29.1',
      //     SUBNET_MASK: document.getElementById('sbc_subnetMask')?.value || '255.255.255.0',
      //     MAC_Address: document.getElementById('sbc_macAddress_ethernet')?.value || '',
      //     DNS_Address: document.getElementById('sbc_dns')?.value || '8.8.8.8',
      //     WIFI_AVAILABLE_LIST: [],
      //     WIFI_SSID: document.getElementById('sbc_wifiSSID')?.value || 'hemant',
      //     WIFI_PASSWORD: document.getElementById('sbc_wifiPassword')?.value || ''
